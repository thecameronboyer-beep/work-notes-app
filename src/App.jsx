...
const styles = {
  ...
  entryTabGrid:{display:"grid",gridTemplateColumns:"repeat(4, 1fr)",gap:6,marginBottom:10},
  ...
  lineGroupTitle:{fontWeight:800,textAlign:"center",fontSize:14,marginBottom:2},
  ...
  reportGrid:{display:"grid",gridTemplateColumns:"24px 50px 52px 30px 1fr 60px",columnGap:2},
...
const normalizeMaterials = (materials) => {
  if (Array.isArray(materials) && materials.length) {
    return materials.map((m) => {
      if (m?.isCoex) {
        return { id:m.id || makeId(), batch:"", die:"", color:"", regrind:"", additive:"", coexes:[{ id:m.id || makeId(), number:m.coexNumber || 1, natural:m.natural || "", color:m.color || "", regrind:m.regrind || "", additive:m.additive || "" }] };
      }
      return {
        id:m?.id || makeId(),
        batch:m?.batch || "",
...
export default function App(){
  ...
  const materialRowsForReport=(line,item)=>{
    ...
    item.materials.forEach((m,matIndex)=>{
      const hasMain=(m.batch||"").trim() || (m.die||"").trim() || (m.color||"").trim() || (m.regrind||"").trim() || (m.additive||"").trim() || (m.coexes||[]).length;
      if(!hasMain) return;
